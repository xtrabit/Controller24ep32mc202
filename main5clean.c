...
void __attribute__((interrupt, no_auto_psv)) _T4Interrupt() // Gyro_value calcs and value update timer; 2ms period; update every 20ms
{
    _T4IF = 0;
    if (gyro_inc < 9)
    {
        ...
        if (gyro_update == 1)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (turn_deg >= 0)
                    {
                        if ((turn_deg / 10 + brake_estimate) > 1800)
                        {
                            //LED_G = 0;
                            MOTOR_L = PDC_cen;
                            MOTOR_R = PDC_cen;
