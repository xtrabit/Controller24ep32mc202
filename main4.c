...
void __attribute__ ( ( interrupt, no_auto_psv ) ) _T5Interrupt (  )             // test timer
    {
    //test_timer_lsb = TMR2;
        ...
        if (gyro_update == 1)
        {
            ...
            else
            {
                if (uturn_counter < 16)
                {
                    if (turn_dir == 1)
                    {
                        MOTOR_L = PDC_cen + (RCvalue_min / 10) * (uturn_counter - 5);
                        MOTOR_R = PDC_cen + (RCvalue_min / 10) * (uturn_counter - 5);
                    }
                    else
                    {
                        MOTOR_L = PDC_cen + (RCvalue_max / 10) * (uturn_counter - 5);
                        MOTOR_R = PDC_cen + (RCvalue_max / 10) * (uturn_counter - 5);
                    }
                ...
                else
                {
                    ...
                    if (turn_dir == 1)
                    {
                        brake_estimate = (long)gyro_value * gyro_value / 8196; 
                        if ((turn_deg + brake_estimate) > 175)
                        {
                            MOTOR_L = PDC_cen;
                            MOTOR_R = PDC_cen;
                        }
...
void angle_tracker(void)
{
    if (gyro_update == 1)
    {
        ...
        turn_deg = turn_deg + (long)gyro_value * 1464/100000;
        gyro_update = 0;
    }
}